...
class LogisticRegression(nn.Module):
    ...
    def __init__(self, n_classes, n_features, **kwargs):
        ...
        # we don't need activation layer since CrossEntropy does that for us
...
def plot(epochs, plottable, ylabel='', name=''):
    ...
    plt.title(name)
    plt.savefig(f'./test/{name}.png', bbox_inches='tight')
    # plt.show()
...
def main():
    ...
    # plot
    plot(epochs, train_mean_losses, ylabel='Loss', name=f'loss{fig_id(opt)}')
    plot(epochs, valid_accs, ylabel='Accuracy', name=f'acc{fig_id(opt)}')

def fig_id(opt):
    res = ""
    res += f"_lr-{opt.learning_rate}"
    res += f"_hidsz-{opt.hidden_sizes}"
    res += f"_dropout-{opt.dropout}"
    res += f"_activ-{opt.activation}"
    res += f"_optim-{opt.optimizer}"
    res += f"_nlayers-{opt.layers}"
    return res
