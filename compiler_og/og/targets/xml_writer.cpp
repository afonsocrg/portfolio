...
static std::string qualifier(int qualifier) {
...
static std::string writeName(bool newLine) {
  if(newLine) return "writeln";
  else return "write";
}
...
void og::xml_writer::do_nullptr_node(og::nullptr_node *const node, int lvl) {
  os() << std::string(lvl,' ') << "<nullptr/>" << std::endl;
}
...
void og::xml_writer::do_alloc_node(og::alloc_node *const node, int lvl) {
  do_unary_operation(node, lvl);
}
...
void og::xml_writer::do_function_declaration_node(og::function_declaration_node *const node, int lvl) {
  // ASSERT_SAFE_EXPRESSIONS
  openTag(node, lvl);
  os() << std::string(lvl+2, ' ') << "<qualifier>" << qualifier(node->qualifier()) << "</qualifier>" << std::endl;
  os() << std::string(lvl+2, ' ') << "<type>" << cdk::to_string(node->type()) << "</type>" << std::endl;
...
void og::xml_writer::do_return_node(og::return_node *const node, int lvl) {
  openTag(node, lvl);
  //if this segfaults, add empty tuple to possible return value
...
void og::xml_writer::do_variable_declaration_node(og::variable_declaration_node *const node, int lvl) {
  ...
  for(std::string name : node->names()) {
    os() << std::string(lvl+4, ' ') << "<name name=" << name << "/>" << std::endl;  
  }
  closeTag("names", lvl+2);
  if(node->initializer()) {
...
void og::xml_writer::do_write_node(og::write_node * const node, int lvl) {
  //ASSERT_SAFE_EXPRESSIONS;
  openTag(writeName(node->newline()), lvl);
  node->arguments()->accept(this, lvl + 2);
  closeTag(writeName(node->newline()), lvl);
}
...
void og::xml_writer::do_ptr_indexation_node(og::ptr_indexation_node *const node, int lvl) {
  openTag(node, lvl);
  openTag("base", lvl+2);
...
void og::xml_writer::do_tpl_indexation_node(og::tpl_indexation_node *const node, int lvl) {
  openTag(node, lvl);
  openTag("base", lvl+2);
  node->base()->accept(this, lvl+4);
  closeTag("base", lvl+2);
